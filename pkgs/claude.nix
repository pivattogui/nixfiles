...
let
  ...
  # Claude settings with OMC plugin enabled
  claudeSettings = builtins.toJSON {
    ...
    statusLine = {
      type = "command";
      command = "node ${homeDir}/.claude/hud/omc-hud.mjs";
    };
    env = {
  ...
  # OMC plugin metadata
  omcPluginMeta = builtins.toJSON {
    version = 2;
    plugins = {
      "oh-my-claudecode@omc" = [
        {
          scope = "user";
          installPath = "${homeDir}/.claude/plugins/cache/omc/oh-my-claudecode/latest";
          version = "latest";
          isLocal = true;
        }
      ];
  ...
  omcHudScript = ''
    ...
    async function main() {
        ...
        try {
          ...
          if (versions.length > 0) {
            ...
            if (builtVersions.length > 0) {
              builtVersions.sort((a, b) => b.localeCompare(a, undefined, { numeric: true }));
              pluginCacheVersion = builtVersions[0];
              pluginCacheDir = join(pluginCacheBase, pluginCacheVersion);
      ...
      // 4. Final fallback - output minimal status
      console.log(JSON.stringify({ text: "OMC" }));
    }
  ...
  # CLAUDE.md content with OMC instructions
  claudeMdContent = ''
    <!-- OMC:START -->
    <!-- OMC:VERSION:latest -->
...
in
{
  # CLAUDE.md with OMC instructions and user preferences
  home.file.".claude/CLAUDE.md".text = claudeMdContent;

  # Claude settings with OMC plugin enabled
  home.file.".claude/settings.json".text = claudeSettings;

  # OMC configuration
  home.file.".claude/.omc-config.json".text = omcConfig;

  # HUD script for statusline
  home.file.".claude/hud/omc-hud.mjs" = {
    text = omcHudScript;
    executable = true;
  };
  ...
  home.activation.claudeConfig = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    config_file="${homeDir}/.claude.json"
    if [ ! -f "$config_file" ]; then
      echo '${claudeConfig}' > "$config_file"
    fi

    # Create plugin directories
    mkdir -p "${homeDir}/.claude/plugins/cache/omc/oh-my-claudecode"
    mkdir -p "${homeDir}/.claude/plugins/oh-my-claudecode"

    # Create installed_plugins.json
    plugins_file="${homeDir}/.claude/plugins/installed_plugins.json"
    if [ ! -f "$plugins_file" ]; then
      echo '${omcPluginMeta}' > "$plugins_file"
    fi
  '';

  # Script to install/update OMC plugin
  home.activation.installOmcPlugin = lib.hm.dag.entryAfter [ "claudeConfig" ] ''
    # Install oh-my-claudecode plugin via npm if not already installed
    omc_cache="${homeDir}/.claude/plugins/cache/omc/oh-my-claudecode"

    # Check if plugin needs installation
    if [ ! -d "$omc_cache/latest" ] && [ -z "$(ls -A "$omc_cache" 2>/dev/null)" ]; then
      ...
      # Install the package
      cd "$temp_dir"
      ${pkgs.nodejs}/bin/npm pack oh-my-claude-sisyphus 2>/dev/null || true

      if [ -f oh-my-claude-sisyphus-*.tgz ]; then
        mkdir -p "$omc_cache/latest"
        tar -xzf oh-my-claude-sisyphus-*.tgz -C "$omc_cache/latest" --strip-components=1
