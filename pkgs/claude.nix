...
in
{
  ...
  home.activation.claudeConfig = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    mcp_config='${claudeConfig}'
    settings_config='${claudeSettings}'

    # Create/update .claude.json (MCP servers)
    if [ ! -f "$config_file" ]; then
      echo "$mcp_config" > "$config_file"
    else
      ${pkgs.jq}/bin/jq -s '.[0] * .[1]' "$config_file" <(echo "$mcp_config") > "$config_file.tmp" && mv "$config_file.tmp" "$config_file"
    fi

    # Create/update settings.json (Claude Code settings)
    if [ ! -f "$settings_file" ]; then
      echo "$settings_config" > "$settings_file"
    else
      ${pkgs.jq}/bin/jq -s '.[0] * .[1]' "$settings_file" <(echo "$settings_config") > "$settings_file.tmp" && mv "$settings_file.tmp" "$settings_file"
    fi
  '';
}
